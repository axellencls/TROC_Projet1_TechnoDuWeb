...
function rechercheBienParMotCle(db, motCle, resServices, callback){
    let resB = [] ;
    //Rechercher les biens par mots clés pb car le champ mot clé du JSON est un tableau
    for( i<4)
    {
	db.collection("Biens").find("motCle":motCle[i]""$elemMatch:"").toArray((err, documents)=> {
		resR.push(documents[0]);
		callback(JSON.stringify(resR));
	});
    }
